...
async function main() {
    console.log('[GovProof Seed] Initializing deterministic, idempotent government testbed data...');
    ...
    for (const name of roleNames) {
        const r = await prisma.role.upsert({
            where: { name },
            update: { description: `Official ${name} role with RBAC permissions` },
            create: { name, description: `Official ${name} role with RBAC permissions` }
        });
        roles[name] = r.id;
    }

    // 2. DEPARTMENTS (Idempotent Upsert)
    console.log('2. Ensuring Departments...');
    const deptData = [
    ...
    // 3. STARTUPS (Idempotent Upsert)
    console.log('3. Ensuring Startups...');
    const startupsData = [
        {
            name: 'MedFlow AI',
            domain: 'Healthcare',
    ...
    for (const s of startupsData) {
        ...
        if (existing) {
            const row = await prisma.startup.update({ where: { id: existing.id }, data: s });
    ...
    // 4. USERS (Idempotent Upsert)
    console.log('4. Ensuring Users...');
    const officerHealth = await prisma.user.upsert({
        where: { email: 'gov@demo.com' },
        update: {
    ...
    const officerUrban = await prisma.user.upsert({
        where: { email: 'officer.urban@demo.com' },
        update: {
    ...
    const expert = await prisma.user.upsert({
        ...
        update: { name: 'Dr. Arvind Kulkarni (Senior Panelist)', roleId: roles['Expert'], password: pwd },
        create: {
    ...
    const validator = await prisma.user.upsert({
        where: { email: 'validator@demo.com' },
        update: { name: 'Sovereign Audit Authority (MSIS Auditor)', roleId: roles['Validator'], password: pwd },
        create: {
    ...
    const medflowUser = await prisma.user.upsert({
        where: { email: 'founder@medflow.com' },
        update: {
    ...
    const wastezeroUser = await prisma.user.upsert({
        where: { email: 'founder@wastezero.com' },
        update: {
    ...
    await prisma.user.upsert({
        ...
        update: { name: 'GovProof Sovereign Admin', roleId: roles['Admin'], password: pwd },
        create: {
    ...
    // 5. CHALLENGES (Idempotent Lookup & Upsert)
    console.log('5. Ensuring Challenges...');
    ...
    for (const c of challengesData) {
        ...
        let row = await prisma.challenge.findFirst({
            where: { title: data.title, departmentId: data.departmentId }
        });
        if (row) {
            row = await prisma.challenge.update({ where: { id: row.id }, data });
        } else {
            row = await prisma.challenge.create({ data });
        }
        challenges[key] = row.id;

        // Challenge Requirements
        const existingReqs = await prisma.challengeRequirement.findMany({ where: { challengeId: row.id } });
        ...
        // Challenge KPIs
        const existingKPIs = await prisma.challengeKPI.findMany({ where: { challengeId: row.id } });
    ...
    // 6. APPLICATIONS (Idempotent Upsert)
    console.log('6. Ensuring Applications...');
    async function getOrCreateApp(challengeId: string, startupId: string, appData: any) {
        let app = await prisma.application.findFirst({ where: { challengeId, startupId } });
        if (app) {
            return await prisma.application.update({ where: { id: app.id }, data: appData });
        }
        return await prisma.application.create({ data: { challengeId, startupId, ...appData } });
    }

    const appMedFlow = await getOrCreateApp(challenges['C_HOSPITAL'], startups['MedFlow AI'], {
    ...
    await getOrCreateApp(challenges['C_HOSPITAL'], startups['CivicPulse Technologies'], {
    ...
    const appWasteZero = await getOrCreateApp(challenges['C_WASTE'], startups['WasteZero Technologies'], {
    ...
    await getOrCreateApp(challenges['C_WASTE'], startups['EcoGrid Innovations'], {
    ...
    const appInfraRoad = await getOrCreateApp(challenges['C_ROAD'], startups['InfraWatch Systems'], {
    ...
    await getOrCreateApp(challenges['C_WATER'], startups['WaterSense Labs'], {
    ...
    // 7. EVALUATIONS (Idempotent Upsert)
    console.log('7. Ensuring Expert Evaluations...');
    async function getOrCreateEval(applicationId: string, expertId: string, evalData: any, criteria: any[]) {
        let ev = await prisma.evaluation.findFirst({ where: { applicationId, expertId } });
        if (ev) {
            ev = await prisma.evaluation.update({ where: { id: ev.id }, data: evalData });
        } else {
            ev = await prisma.evaluation.create({ data: { applicationId, expertId, ...evalData } });
        }
        const existingScores = await prisma.evaluationScore.findMany({ where: { evaluationId: ev.id } });
        if (existingScores.length === 0) {
            for (const c of criteria) {
                await prisma.evaluationScore.create({ data: { evaluationId: ev.id, ...c } });
            }
        }
        return ev;
    }

    await getOrCreateEval(appMedFlow.id, expert.id, {
        status: 'SUBMITTED',
        overallScore: 93,
    ...
    await getOrCreateEval(appWasteZero.id, expert.id, {
        status: 'SUBMITTED',
        overallScore: 91,
    ...
    // 8. PILOTS (Idempotent Upsert)
    console.log('8. Ensuring Pilots...');
    async function getOrCreatePilot(challengeId: string, startupId: string, pilotData: any) {
        let pilot = await prisma.pilotProject.findFirst({ where: { challengeId, startupId } });
        if (pilot) {
            return await prisma.pilotProject.update({ where: { id: pilot.id }, data: pilotData });
        }
        return await prisma.pilotProject.create({ data: { challengeId, startupId, ...pilotData } });
    }

    const pilotHospital = await getOrCreatePilot(challenges['C_HOSPITAL'], startups['MedFlow AI'], {
    ...
    const pilotWaste = await getOrCreatePilot(challenges['C_WASTE'], startups['WasteZero Technologies'], {
    ...
    const pilotRoad = await getOrCreatePilot(challenges['C_ROAD'], startups['InfraWatch Systems'], {
    ...
    // 9. PILOT KPIS (Idempotent)
    console.log('9. Ensuring Pilot KPIs...');
    async function getOrCreatePilotKPI(pilotId: string, name: string, data: any) {
        let kpi = await prisma.pilotKPI.findFirst({ where: { pilotId, name } });
        if (kpi) return await prisma.pilotKPI.update({ where: { id: kpi.id }, data });
        return await prisma.pilotKPI.create({ data: { pilotId, name, ...data } });
    }

    const kpiWaitTime = await getOrCreatePilotKPI(pilotHospital.id, 'Average Outpatient Waiting Time', {
    ...
    await getOrCreatePilotKPI(pilotHospital.id, 'Patient Consultation Throughput', {
    ...
    const kpiCSAT = await getOrCreatePilotKPI(pilotHospital.id, 'Patient Satisfaction Index (CSAT)', {
    ...
    const kpiWasteCost = await getOrCreatePilotKPI(pilotWaste.id, 'Monthly Fleet Fuel OPEX', {
    ...
    await getOrCreatePilotKPI(pilotWaste.id, 'Collection Efficiency Rate', {
    ...
    await getOrCreatePilotKPI(pilotRoad.id, 'Defect-to-Repair Cycle', {
    ...
    // 10. PILOT MILESTONES & PAYMENTS (Idempotent)
    console.log('10. Ensuring Milestones & Payments...');
    async function getOrCreateMilestone(pilotId: string, title: string, data: any) {
        let m = await prisma.pilotMilestone.findFirst({ where: { pilotId, title } });
        if (m) return await prisma.pilotMilestone.update({ where: { id: m.id }, data });
        return await prisma.pilotMilestone.create({ data: { pilotId, title, ...data } });
    }

    const mHosp1 = await getOrCreateMilestone(pilotHospital.id, 'Milestone 1: Telemetry Hardware & EMR API Gateway Setup', {
    ...
    const mHosp2 = await getOrCreateMilestone(pilotHospital.id, 'Milestone 2: 60-Day Active Live Telemetry & Queue Optimization', {
    ...
    const mHosp3 = await getOrCreateMilestone(pilotHospital.id, 'Milestone 3: Final Pilot Outcome & Verification Audit Report', {
    ...
    // Payments (Idempotent check by milestoneId)
    async function getOrCreatePayment(pilotId: string, milestoneId: string, payData: any) {
        let p = await prisma.payment.findFirst({ where: { pilotId, milestoneId } });
        if (p) return await prisma.payment.update({ where: { id: p.id }, data: payData });
        return await prisma.payment.create({ data: { pilotId, milestoneId, ...payData } });
    }

    await getOrCreatePayment(pilotHospital.id, mHosp1.id, {
        amount: 1000000,
        status: 'PAID',
    ...
    await getOrCreatePayment(pilotHospital.id, mHosp2.id, {
        amount: 1500000,
        status: 'PAID',
    ...
    await getOrCreatePayment(pilotHospital.id, mHosp3.id, {
        amount: 1000000,
        status: 'APPROVED',
        dueDate: new Date(Date.now() + 5 * 24 * 3600 * 1000)
    });

    // 11. EVIDENCE VAULT (Idempotent)
    console.log('11. Ensuring Evidence Vault...');
    async function getOrCreateEvidence(pilotId: string, title: string, data: any) {
        let e = await prisma.evidence.findFirst({ where: { pilotId, title } });
        if (e) return await prisma.evidence.update({ where: { id: e.id }, data });
        return await prisma.evidence.create({ data: { pilotId, title, ...data } });
    }

    await getOrCreateEvidence(pilotHospital.id, 'Hospital OPD Baseline & API Telemetry Integration Certificate', {
    ...
    await getOrCreateEvidence(pilotHospital.id, '90-Day Raw Queue Telemetry & Time-in-Motion Dataset', {
    ...
    await getOrCreateEvidence(pilotHospital.id, 'Patient Exit CSAT Kiosk Survey Analytics & Video Audit', {
    ...
    await getOrCreateEvidence(pilotWaste.id, 'Pune Municipal Zone 4 Fuel Telemetry Log & Weighbridge Audit', {
    ...
    await getOrCreateEvidence(pilotRoad.id, 'Thane Highway LiDAR Road Defect Scan & Contractor Work Orders', {
    ...
    // 12. VALIDATION RECORDS (Idempotent)
    console.log('12. Ensuring Validation Records...');
    async function getOrCreateValidation(pilotId: string, validatorId: string, data: any) {
        let v = await prisma.validationRecord.findFirst({ where: { pilotId, validatorId } });
        if (v) return await prisma.validationRecord.update({ where: { id: v.id }, data });
        return await prisma.validationRecord.create({ data: { pilotId, validatorId, ...data } });
    }

    await getOrCreateValidation(pilotHospital.id, validator.id, {
        decision: 'VALIDATED',
    ...
    await getOrCreateValidation(pilotWaste.id, validator.id, {
        decision: 'VALIDATED',
    ...
    await getOrCreateValidation(pilotRoad.id, validator.id, {
        decision: 'REJECTED',
    ...
    // 13. PROOF PASSPORTS (Idempotent Upsert by passportNumber)
    console.log('13. Ensuring Innovation Proof Passports...');
    ...
    await prisma.proofPassport.upsert({
        where: { passportNumber: 'GPP-MH-2026-0042' },
        update: {
    ...
    await prisma.proofPassport.upsert({
        where: { passportNumber: 'GPP-MH-2026-0018' },
        update: {
    ...
    // 14. SCALE DECISIONS & PROCUREMENT (Idempotent Upsert)
    console.log('14. Ensuring Scale Decisions & Procurement Records...');
    const scaleMedFlow = await prisma.scaleDecision.upsert({
        where: { pilotId: pilotHospital.id },
        update: {
    ...
    await prisma.procurementRecord.upsert({
        where: { scaleDecisionId: scaleMedFlow.id },
        update: {
    ...
    const scaleWaste = await prisma.scaleDecision.upsert({
        where: { pilotId: pilotWaste.id },
        update: {
    ...
    await prisma.procurementRecord.upsert({
        where: { scaleDecisionId: scaleWaste.id },
        update: {
    ...
    await prisma.scaleDecision.upsert({
        where: { pilotId: pilotRoad.id },
        update: {
            decisionById: officerUrban.id,
            recommendation: 'STOP',
            finalDecision: 'STOP',
    ...
    // 15. AUDIT LOGS (Idempotent: seed initial audit if empty)
    console.log('15. Ensuring Audit Trail...');
    ...
    console.log('[GovProof Seed] Complete! Database successfully verified and populated with deterministic testbed data.');
}
